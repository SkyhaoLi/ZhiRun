...
//                   input_dev_num);
    ...
    for (i = 0; i < input_dev_num; i++)
    {
        ...
        if (strstr(tp_name, "ts") || strstr(tp_name, "gsl") || strstr(tp_name, "ADS"))
        {
...
void evdev_read(lv_indev_t *drv, lv_indev_data_t *data)
{
    ...
    {
        // 翻转标志
        int flip_x = 1; // 1 = 翻转, 0 = 不翻转
        if (flip_x)
            raw_x = map(raw_x, evdev_min_x, evdev_max_x, hor_res, 0);
        else
            raw_x = map(raw_x, evdev_min_x, evdev_max_x, 0, hor_res);

        int flip_y = 0; // 根据需要翻转Y轴
        if (flip_y)
            raw_y = map(raw_y, evdev_min_y, evdev_max_y, ver_res, 0);
        else
